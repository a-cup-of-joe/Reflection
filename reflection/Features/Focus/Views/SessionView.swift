                ...
                Button(action: {
                    ...
                    .background(
                        RoundedRectangle(cornerRadius: CornerRadius.large)
                            ...
                            .shadow(color: Color.primaryGreen.opacity(0.3), radius: isPressed ? 8 : 16, x: 0, y: isPressed ? 4 : 8)
                    )
            ...
            if !sessionViewModel.sessions.isEmpty {
                VStack(alignment: .leading, spacing: Spacing.lg) {
                    ...
                    LazyVStack(spacing: Spacing.md) {
                        ForEach(sessionViewModel.sessions.suffix(3).reversed(), id: \.id) { session in
...
struct TaskSelectionPanel: View {
        ...
        HStack(spacing: 0) {
            ...
            TaskCustomizationArea(
                ...
                onStart: {
                    let project = selectedPlan?.project ?? customProject
                    let themeColor = selectedPlan?.themeColor ?? "#00CE4A"
                    let expectedTimeInterval = parseTimeToSeconds(expectedTime)
                    onStartSession(project, taskDescription, themeColor, expectedTimeInterval, goals.filter { !$0.isEmpty })
                }
            )
        }
        .background(Color.appBackground)
    ...
    private func parseTimeToSeconds(_ timeString: String) -> TimeInterval? {
        guard !timeString.isEmpty else { return nil }
        
        // 支持格式：30m, 1h, 1h30m, 90m 等
        let pattern = #"(\d+)([hm])"#
        let regex = try? NSRegularExpression(pattern: pattern)
        let matches = regex?.matches(in: timeString, range: NSRange(timeString.startIndex..., in: timeString)) ?? []
        
        var totalSeconds: TimeInterval = 0
        
        for match in matches {
            if let numberRange = Range(match.range(at: 1), in: timeString),
               let unitRange = Range(match.range(at: 2), in: timeString) {
                let number = Int(timeString[numberRange]) ?? 0
                let unit = String(timeString[unitRange])
                
                switch unit {
                case "h":
                    totalSeconds += TimeInterval(number * 3600)
                case "m":
                    totalSeconds += TimeInterval(number * 60)
                default:
                    break
                }
            }
...
struct ActiveSessionPanel: View {
    ...
    var body: some View {
        ...
        .background(
            ...
            LinearGradient(
                gradient: Gradient(colors: [
                    session.themeColorSwiftUI.opacity(0.8),
                    session.themeColorSwiftUI.opacity(0.6),
                    session.themeColorSwiftUI.opacity(0.9)
                ]),
                startPoint: .topLeading,
                endPoint: .bottomTrailing
            )
